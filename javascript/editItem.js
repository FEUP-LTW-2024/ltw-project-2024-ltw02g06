...
const redirectToUrlWithRedirectParam = () => {
  ...
  if (redirectParam) {
    ...
    var redirectUrl = "/pages/item.php?id=" + getQueryParam("id");
    window.location.href = redirectUrl;
  }
};
...
const handleEditItemCategoryList = (item, categories) => {
  ...
  const updateAttributes = () => {
    ...
    for (const attributeId in selectedCategory.attributes) {
      ...
      // Set value if category matches item's category
      if (selectedCategoryId == item.category) {
        const itemAttribute = item.attributes[attributeId];
        if (itemAttribute) {
          if (attribute.type == "enum") {
            inputElement.value = itemAttribute.value;
          } else {
            inputElement.value = itemAttribute.value || "";
          }
        }
